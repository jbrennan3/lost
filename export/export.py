...
def main():
    ...
    SQL = "SELECT asset_tag, description, common_name, arrive_dt, dispose_dt FROM facilities f JOIN asset_at aa ON f.facility_pk=aa.facility_fk JOIN assets a ON a.asset_pk=aa.asset_fk;"
    cur.execute(SQL)
    results = cur.fetchall()
    ...
    with open(datapath + '/transfers.csv', "w") as csvfile:
            fieldnames = ['asset_tag', 'requester', 'request_dt', 'approver', 'approve_dt', 'source', 'destination', 'load_dt', 'unload_dt']
            writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
            ...
            for row in results:
                ...
                dest = cur.fetchone()[0]
                writer.writerow({'asset_tag' : row[0], 'requester': row[1], 'request_dt': row[2], 'approver': row[3], 'approve_dt': row[4], 'source': row[5], 'destination': dest, 'load_dt': row[7], 'unload_dt': row[8]}) 
